...
fun BuyCryptoScreen(
    ...
    Column(
        ...
        var orders by remember { mutableStateOf(listOf<PlacedOrderItem>()) }
        LaunchedEffect(Unit) {
            orders = Network.fetchActiveOrders()
            isLoading = false
        }
        ...
        LazyColumn {
            items(orders.size) { index ->
                ...
                OrderItem(
                    ...
                    amountAsked = orders[index].fiatAmountAsked.toString(),
                    tokensProvided = "0.000000000000001",
                    current
                )
    ...
    Card(
        ...
        Column(
            ...
            Row(
                ...
                Text(
                    text = "Tokens Provided: $tokensProvided shido",
                )
                Spacer(modifier = Modifier.width(8.dp))
                AsyncImage(model = ChainInfo.Ethereum.icon, contentDescription = null, modifier = Modifier.size(24.dp))
            }
